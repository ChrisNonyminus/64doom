...
//
//
int NetbufferSize(void) { return sizeof(doomdata_t); }
...
boolean HGetPacket(void)
{
    ...
    if ((NetbufferChecksum() & NCMD_CHECKSUM) !=
        (netbuffer->checksum & NCMD_CHECKSUM))
    {
        if (true)
            printf("bad packet checksum (netbuffer->checksum: %08X | "
                   "NetbufferChecksum(): %08X) \n",
...
void GetPackets(void)
{
    ...
    while (HGetPacket())
    {
        ...
        //     I_Error ("GetPackets: Killed by network driver (checksum %08X)",
        //     netbuffer->checksum);
    ...
    for (i = 0; i < doomcom->numnodes; i++)
    {
        if (nodeingame[i])
        {
            ...
            if (netbuffer->numtics > BACKUPTICS)
                I_Error("NetUpdate: netbuffer->numtics (aka %d) > BACKUPTICS "
                        "(aka %d)",
...
void CheckAbort(void)
{
    ...
    //    for ( ; eventtail != eventhead ; eventtail =
        ...
        while (1)
        {
            ...
            if (netbuffer->checksum & NCMD_SETUP)
            {
                if (netbuffer->player != VERSION)
                    I_Error("D_ArbitrateNetStart: Different DOOM versions "
            ...
            else
            {
                ...
                printf(
                    "still waiting on packet; checksum (netbuffer->checksum: "
                    "%08X | NetbufferChecksum(): %08X) \n",
                    netbuffer->checksum, NetbufferChecksum());
            }
        }
    }
